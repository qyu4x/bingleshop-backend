...
describe("Payment Method Service - create", () => {
  ...
  it("should create a new payment method", async () => {
    ...
    expect(paymentMethodRepository.findOneByName).toHaveBeenCalledWith(
      "Test Payment Method"
    )
  ...
  it("should throw error if payment method already exists", async () => {
    ...
    await expect(createPaymentMethodService(mockRequest)).rejects.toThrow(
      ResponseError
    )
    expect(paymentMethodRepository.findOneByName).toHaveBeenCalledWith(
      "Test Payment Method"
...
describe("remove payment", () => {
  it("should remove a payment method", async () => {
    ...
    expect(paymentMethodRepository.findOneById).toHaveBeenCalledWith(
      PaymentMethodId
    )
  })
  ...
  it("should throw error if payment method not found", async () => {
    ...
    await expect(paymentMethodService.remove(PaymentMethodId)).rejects.toThrow(
      ResponseError
    )
    expect(paymentMethodRepository.findOneById).toHaveBeenCalledWith(
      PaymentMethodId
